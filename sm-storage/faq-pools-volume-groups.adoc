...
+
Para crear manualmente un pool: selecciona *Almacenamiento* y luego selecciona *Pools y grupos de volúmenes*. Desde la pestaña Toda la capacidad, haz clic en menú:Crear[Pool].
...
+
Para agregar manualmente unidades a un pool existente: en la página Pools y grupos de volúmenes, selecciona el pool y luego haz clic en *Añadir capacidad*.
...
Puedes cambiar la capacidad de conservación desde *Pools y grupos de volúmenes*. Selecciona el pool que quieres editar. Haz clic en el menú:icono de elipsis[Editar configuración] y luego selecciona la pestaña *Configuración*.
...
En el cuadro de diálogo Add capacity, no todas las unidades están disponibles para añadir capacidad a un pool o grupo de volúmenes existente.
...
** Debes tener un número par de unidades en el grupo de volúmenes. Si no tienes un número par de unidades y te quedan algunas unidades sin asignar, ve a *Pools and volume groups* para añadir unidades adicionales al grupo de volúmenes y vuelve a intentar la operación.
...
Para ver las estadísticas de la caché SSD, selecciona menú:Storage[Pools and volume groups] desde el panel izquierdo. Selecciona la caché SSD para la que quieres ver las estadísticas y luego haz clic en el menú:ellipsis icon[View statistics]. Las estadísticas nominales se muestran en la página View Statistics.
...
[NOTE]
====
El control deslizante Additional Optimization Capacity solo está disponible para los sistemas de almacenamiento EF80, EF50, EF600 y EF300.
...
El aprovisionamiento de recursos es una función disponible en las matrices de almacenamiento EF80, EF50, EF300 y EF600, que permite poner en uso volúmenes de forma inmediata sin proceso de inicialización en segundo plano.
...
El aprovisionamiento de recursos está activado por defecto en los sistemas en los que las unidades admiten DULBE. Puedes desactivar esa configuración por defecto desde *Pools y grupos de volúmenes*.
