...
. Haz clic en el icono de menú:ellipsis[Change volume cache settings].
+
Aparece la ventana Change volume cache Settings. Todos los volúmenes de la storage array aparecen en este cuadro de diálogo.

. Marca la casilla junto a cada volumen para el que quieras cambiar la configuración de caché o marca la casilla en el encabezado de la tabla para seleccionar todos los volúmenes.
+
[cols="25h,~"]
|===
...
En el caso de volumen fino, la captura previa de la lectura de caché dinámica siempre está deshabilitada y no se puede modificar.
 ...
 a| 
Caché de escritura sin baterías
 a| 
...
|===
. Haz clic en *Guardar* para cambiar la configuración de la caché
