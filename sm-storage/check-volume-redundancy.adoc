...
NOTE: La función Check Volume Redundancy actualmente solo está disponible para operaciones en la interfaz de Legacy System Manager.

. En el panel izquierdo, selecciona menú:Storage[Pools y grupos de volúmenes].
. Haz clic en el icono de menú:ellipsis[Check volume redundancy] para el pool o grupo de volúmenes que quieras comprobar.
+
